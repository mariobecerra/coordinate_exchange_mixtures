...
optimize_cox_direction_old_cpp = function(
  X_in, k, i, log_d_eff_best,
  n_cox_points = 100,
  order = 1,
  method = NULL,
...
coord_ex_mixt_old_cpp = function(n_runs = 10, q = 3, n_cox_points = 100, order = 1, max_it = 50, seed = NULL, X = NULL, plot_designs = F, verbose = 1, method = "finite_approximation"){
  ...
  while(it < max_it){
    ...
    for(k in 1:nrow(X)){
      ...
      for(i in 1:q){
        ...
        optim_list = optimize_cox_direction_old_cpp(
          X_in = X, 
          k = k, 
          i = i, 
          order = order,
